...
function Game({ gameCode, gameData, myRole, pseudo, socket }) {
  ...
  if (!gameData || !myRole) {
    return (
      <div className="container">
        <div className="loading">{t('game.initializing')}</div>
      </div>
    );
  }

  const teamEmoji = myRole.team === 'bleu' ? '🔵' : '🔴';
  const teamName = myRole.team === 'bleu' ? t('game.teamBlue') : t('game.teamRed');

  const getRoleDescription = (role) => {
    switch (role) {
      case 'representant':
        return `👑 ${t('game.role.representant')}`;
      case 'tueur':
        return `🔪 ${t('game.role.tueur')}`;
      case 'lambda':
        return `👤 ${t('game.role.lambda')}`;
      default:
        return role;
    }
  };
  ...
  const actualTeam = myRole.isTraitor ? t('game.traitors') : teamName;
    ...
    <div className="logo-circle"><img src="/logo-bvr.png" alt="Logo Blue vs Red" className="logo-img" /></div>
      ...
      <div className="role-card">
        ...
        {myRole.isTraitor ? (
          <>
            ...
            <div style={{ fontSize: '14px', color: '#999', marginTop: '5px' }}>
              {t('game.yourTeam')} : {teamEmoji} {teamName}
            </div>
            ...
            {myRole.traitorInfo && (
              <div style={{ marginTop: '15px', padding: '10px', background: 'rgba(139, 0, 255, 0.1)', borderRadius: '8px' }}>
                <div style={{ fontSize: '14px', color: '#8B00FF', marginBottom: '5px' }}>🤝 {t('lobby.traitors')} :</div>
                <div style={{ fontSize: '13px' }}>
                  <strong>{t('game.players')} {myRole.traitorInfo.anonymousNumber}</strong>
                </div>
                <div style={{ fontSize: '12px', color: '#666' }}>
                  {t('game.yourTeam')} : {myRole.traitorInfo.team === 'bleu' ? '🔵' : '🔴'} {myRole.traitorInfo.team}
                </div>
        ...
        {myRole.munitions > 0 && (
          <div style={{ marginTop: '10px', fontSize: '16px' }}>
            💣 {t('game.role.tueur')} : {myRole.munitions}
          </div>
        )}
      </div>
        ...
        <div style={{
          ...
          {eliminationMessage.eliminated && eliminationMessage.eliminated.map((dead, idx) => (
            <div key={idx} style={{ fontSize: '14px', marginTop: '5px' }}>
              💀 {t('game.players')} {dead.anonymousNumber} - {dead.reason}
            </div>
          ))}
        </div>
      )}
      ...
      {votingPhase === 'DISCUSSION' && voteInfo && (
        <div style={{
          ...
          <h3 style={{ color: 'white', marginBottom: '10px' }}>
            💬 {t('vote.title')} {voteInfo.voteNumber}/{voteInfo.totalVotes}
          </h3>
          <div style={{ fontSize: '14px' }}>
            {t('vote.description')}
          </div>
          <div style={{ fontSize: '12px', marginTop: '5px', opacity: 0.9 }}>
            {t('vote.waiting')} {formatTime(voteInfo.discussionEnd - Date.now())}
          </div>
        </div>
      )}
      ...
      {votingPhase === 'VOTING' && voteInfo && (
        <div style={{
          ...
          <h3 style={{ color: 'white', marginBottom: '10px', textAlign: 'center' }}>
            🗳️ {t('vote.title')} {voteInfo.voteNumber}/{voteInfo.totalVotes}
          </h3>
                ...
                {gameData.players.filter(p => p.isAlive && p.pseudo !== pseudo).map((player, index) => (
                  ...
                  >
                    <div style={{ fontWeight: 'bold' }}>
                      {t('game.players')} {player.anonymousNumber}
                    </div>
          ...
          <div style={{ fontSize: '12px', marginTop: '10px', textAlign: 'center', opacity: 0.9 }}>
            {t('vote.waiting')} {formatTime(voteInfo.votingEnd - Date.now())}
          </div>
        </div>
      )}
      ...
      {gameData.players.some(p => !p.isAlive) && (
        <div style={{
          ...
          <div style={{ fontSize: '18px', marginBottom: '10px' }}>💀 {t('gameEnded.eliminated')}</div>
      ...
      {showChat && (
        <div style={{
          ...
          <div style={{
            ...
            <div>
              <div style={{ fontWeight: 'bold', fontSize: '16px' }}>💬 {t('game.chat')}</div>
              <div style={{ fontSize: '11px', opacity: 0.9 }}>{t('game.players')}</div>
            </div>
            <button
                ...
                <div key={idx} style={{
                  ...
                  <div style={{
                    ...
                    <span style={{ fontWeight: 'bold', color: '#667eea' }}>
                      {t('game.players')} #{msg.playerNumber}
                    </span>
